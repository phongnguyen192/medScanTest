...
var hubItemsURL = "https://niftitusa.sharepoint.com/sites/ExternalSharing/_api/web/lists/getByTitle('Hub')/rootFolder/Folders?$expand=ListItemAllFields,Destination_x0020_Physician";
var doctorsURL = "https://niftitusa.sharepoint.com/sites/ExternalSharing/_api/web/lists/getByTitle('Doctors List')/items?$top=5000";
var doctorsURLExternal = "https://niftitusa.sharepoint.com/sites/ExternalSharing/_api/web/lists/getByTitle('Doctors List')/items?$top=5000&$filter=(External_x0020_Site eq 'Yes')";
...
app.controller("myCtrl", function ($scope, $http, $q, moment, $timeout) {
	...
	$scope.applyAll = function ($event) {
		...
		$scope.loaderDisplay = true; // Change request from client
		...
		if (selectedItems) {
			if (currentPhysician && currentPayment) {
				...
				$q.all(allPromises).then(function (r) {
					updateItemView(r, $scope.items, 1);
	...
	//Fn: Update datasource item after the model update.
	function updateItemView(sources, destination, type) {
		for (var i = 0; i < sources.length; i++) {
			for (var j = 0; j < destination.length; j++) {
				if (sources[i].Id == destination[j].Id) {
					if (type == 1) {
						destination[j].folderPaymentStatus = sources[i].isPaid ? 'Paid' : 'Unpaid';
						destination[j].currentStatus = sources[i].Current_x0020_Status;
						destination[j].destinationPhysician = matchFolderNameToDestinationPhysician(sources[i].Destination_x0020_PhysicianId);
						if (sources[i].Access_x0020_expiration_x0020_date == null) {
							destination[j].accessExpirationDate = null;
						}
					} else if (type == 2) {
						destination[j].currentStatus = sources[i].Current_x0020_Status;
						destination[j].sharedTo = destination[j].SharedTo;
						destination[j].destinationPhysicianLink = destination[j].SharedToURL;
					    destination[j].accessExpirationDate = sources[i].Access_x0020_expiration_x0020_date != null ? new Date(sources[i].Access_x0020_expiration_x0020_date): null;
						console.log(new Date(destination[j].accessExpirationDate))
						$scope.$apply();
					}
				}
			}
	...
	$scope.clickInstantShare = function ($event, item) {
		...
		$pnp.sp.web.lists.getByTitle("Hub").items.getById(item.Id).update(dataModel).then(function () {
			//getRecords().then(function (r) {
			//	reloadPageAndReconfigureDatatable();

			//})
			dataModel.Id = item.Id;
			updateItemView([dataModel], $scope.items, 2);
		})
	}
